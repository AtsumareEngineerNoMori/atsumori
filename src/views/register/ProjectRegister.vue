...
<!-- <HeaderModal /> -->
    ...
    <div class="ProjectRegister-details">
      <div class="ProjectRegister-details-icon">
        ...
        <div class="ProjectRegister-details-name">プロジェクトの名前 
          <input v-model="project.name" type="text" class="ProjectRegister-details-name-input"/></div>
      </div>
  
      <div class="ProjectRegister-details-information">
        <p class="ProjectRegister-details-information-title">プロジェクトの情報</p>
        <textarea v-model="project.description" class="ProjectRegister-details-information-text"></textarea>
      </div>
    </div>
  </div>
  <RouterLink to="/top">
    <button @click="projectRegisterButton" class="ProjectRegister-set-button">登録する</button>
    </RouterLink>
  </div>
  </div>
  </template>
...
const iconImg = vueref("https://firebasestorage.googleapis.com/v0/b/atsumareengineernomori.appspot.com/o/project%2Fha.png?alt=media&token=6141dc6e-714a-4257-91e2-88fcd6d436ad");
...
const projectRegisterButton = (async) => {
        ...
        storage, `project/${iconFileName.value}` );
